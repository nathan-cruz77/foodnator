...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=foodnator.example.com
...
workon foodnator  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development foodnator npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop foodnator
    docker rm foodnator
    docker run --name foodnator -d --env-file /home/ubuntu/foodnator.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/foodnator:/dkdata \
        foodnator start_web.sh
}
